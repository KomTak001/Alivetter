...
Ti.API.debug('★★★ saveAccessToken START');
...
Ti.API.debug('★★★ saveAccessToken END');
    };
...
Ti.API.debug('▲ getRequestToken START');
...
Ti.API.debug('▲ getRequestToken END');
    };
...
Ti.API.debug('★webView.html : ' + webView.html + "★");
//Ti.API.debug('★' + webView.evalJS('window.document') + "★");
// try {
// if(e.url === "https:\/\/api.twitter.com\/oauth\/authorize") {
       var val = webView.evalJS('window.document.querySelector(\'kbd[aria-labelledby="code-desc"] > code\').innerHTML');
		...
		// var htmlSouce = webView.evalJS("document.documentElement.innerHTML");
		// var xmlDocument = Ti.XML.parseString(htmlSouce);
...
// 
			...
			// if (id && id.nodeValue == 'oauth_pin') {
				// var kbdList = node.getElementsByTagName('kbd');
				// var kdb = List.item(0);
				// val = kdb.firstChile.text;
				// break;
			// }
		// }
// 
...
Ti.API.debug('★ showAuthorizeUI START');
...
Ti.API.debug('★ showAuthorizeUI END');
    };

    this.getAccessToken = function(pUrl, pCallback) {
Ti.API.debug('★★ getAccessToken START');
...
Ti.API.debug('★★ getAccessToken END');
    };
